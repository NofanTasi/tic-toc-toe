...
export function getBestAIMove(
  ...
  // 1. Check if AI can win by placing a mark in an empty cell
  for (let i = 0; i < 9; i++) {
    if (board[i] === null) {
      const nextBoard = [...board];
      nextBoard[i] = aiPlayer;
      if (checkWin(nextBoard).winner === aiPlayer) {
        return { action: 'place', index: i };
      }
    }
  }

  // 2. Check if AI must block opponent from winning on their next turn
  for (let i = 0; i < 9; i++) {
    if (board[i] === null) {
      const nextBoard = [...board];
      nextBoard[i] = opponent;
      if (checkWin(nextBoard).winner === opponent) {
        return { action: 'place', index: i };
      }
    }
  }

  // 3. If board is full, AI MUST remove a 3-in-a-line symbol set!
  const filledLines = getFilledLines(board);
  if (isBoardFull(board) || (filledLines.length > 0 && Math.random() < 0.15)) {
    // Check if canonical safe line removals exist
    const safeLines = getSafeLineRemovals(board);
    if (safeLines.length > 0) {
      // Pick a safe line removal
      ...
      // Fallback if no safe match found (e.g. non-full board removal)
      const chosen = filledLines[Math.floor(Math.random() * filledLines.length)];
      return { action: 'clear', line: chosen };
    }
  }

  // 4. Center control (Index 4)
  if (board[4] === null) {
    return { action: 'place', index: 4 };
  }

  // 5. Take corners (0, 2, 6, 8)
  const corners = [0, 2, 6, 8].filter((i) => board[i] === null);
  if (corners.length > 0) {
    const chosenCorner = corners[Math.floor(Math.random() * corners.length)];
    return { action: 'place', index: chosenCorner };
  }

  // 6. Take remaining edges (1, 3, 5, 7)
  const edges = [1, 3, 5, 7].filter((i) => board[i] === null);
  if (edges.length > 0) {
    const chosenEdge = edges[Math.floor(Math.random() * edges.length)];
    return { action: 'place', index: chosenEdge };
  }

  // Fallback if any filled lines exist
  if (filledLines.length > 0) {
    return { action: 'clear', line: filledLines[0] };
  }

  // Absolute fallback
  const remaining = board.map((c, idx) => (c === null ? idx : null)).filter((val) => val !== null) as number[];
  return { action: 'place', index: remaining[0] };
}
